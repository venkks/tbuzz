...
<div class="row"> 
		...
		<%  String currentDomain = request.getParameter("domain");
		 ...
		 &nbsp;<br />&nbsp;&nbsp;<span stlye="color: <spring:message code='subtext.color.row' />;">Items from source: <%=request.getParameter("domain")%> <% User user = (User) session.getAttribute("userInfo"); String domain=request.getParameter("domain"); Boolean isFollow=(request.getAttribute("isFollowAllowed") != null? (Boolean) request.getAttribute("isFollowAllowed"):false); if (isFollow && user != null) { %>
		</span><span class="auto-style15" stlye="color: <spring:message code='subtext.color.row' />;">(<a href="<%=request.getContextPath()%>/FollowManager?op=add&domain=<%=domain%>">Follow</a>)</span>
